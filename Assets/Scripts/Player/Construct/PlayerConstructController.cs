...
public class PlayerConstructController : MonoBehaviour {
  ...
  private void handleInput() {
    ...
    if (pcam.aimedWJ != null) {
      if (controlledConstruct.canInteractWJ(pcam.aimedWJ)) {
        if (!pcam.aimedWJ.isHighlighted()) pcam.aimedWJ.highlight();
    ...
    // [Unhighlight]: Unhover
      ...
      if (pcam.prevAimedWJ.isHighlighted()) pcam.prevAimedWJ.unhighlight();
    ...
    if (Input.GetMouseButtonDown(0)) {
      ...
      if (pcam.aimedWJ != null) pcam.aimedWJ.unhighlight();
      dataViewer.setActive(false);
    }
  }
