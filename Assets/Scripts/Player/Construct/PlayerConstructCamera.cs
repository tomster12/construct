...
public class PlayerConstructCamera : MonoBehaviour {
  ...
  private void Update() {
    ...
    if (followConstruct != null) {
      ...
      if (followConstruct.mainWJ != followPrevWJ) {
        updateCamOffset();
        followPrevWJ = followConstruct.mainWJ;
      }

      // Set pivot to target, and lerp target to offset
      camPivot.position = followConstruct.mainWJ.transform.position;
  ...
  private void updateCamOffset() {
    ...
    float maxExtent = followConstruct.mainWJ.getMaxExtent();
